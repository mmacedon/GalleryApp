#search_artist_birth_year.py
import database_handler
import cgi
import pymysql
import format
import search_artist_country
...
def main():
    ...
    if ( form.getvalue('artist_birth_year')):
        birth_year = int(form.getvalue('artist_birth_year'))

    if ( birth_year == -10 ):
        print("search_artist_birth_year.py main(): Error! Need to input field.")
        return 0
    print("""
        <body>
            <h1> Artists Results with Birth Year %s </h1>
        """ % birth_year)
    database = database_handler.initialize()
    if ( database == -1 ):
        print("search_artist_birth_year.py main(): Error linking database")
        return 0
    cursor = database.cursor()
    query = "SELECT * FROM artist WHERE birth_year = '" + str(birth_year) + "'"
    cursor.execute(query)
    response = cursor.fetchall()
    artists = []
    if (response == None):
        print("No Artists Found With Birth Year %s" % str(birth_year))
        return 0
